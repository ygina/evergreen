    ...
    <div className="waterfall-grid">
      {
        ...
        .map(function(row){
          return <Variant row={row} project={project} collapsed={collapsed} versions={data.versions} taskFilter={taskFilter}/>;
        })
      }
    </div> 
  )
          ...
          <div className="row build-cols">
            {
              row.versions.map((versionId,i) => {
                return <Build key={versionId} build={row.builds[versionId]} version={versions[versionId]} collapsed={collapsed} taskFilter={taskFilter}/>
              })
            }
          </div>
        </div>
...
function Build({build, collapsed, version, taskFilter}){
  ...
  return (
    <div className="build">
      <ActiveBuild build={build} taskFilter={taskFilter}/>
    </div>
  )
}
